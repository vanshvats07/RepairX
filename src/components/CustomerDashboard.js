...
export default function CustomerDashboard({ user }) {
  ...
  async function startInvestigation(event) {
    ...
    try {
      ...
      if (!analysisResponse.ok) throw new Error("The investigation was created, but its preliminary analysis could not be completed.");
  ...
  return <main className="data-shell">
    <header className="data-header"><div><WorkspaceNav items={["Overview", "My Devices", "My Repairs", "Repair DNA"]} /><span className="section-label">CUSTOMER WORKSPACE</span><h1>Welcome back, {user?.name || "there"}.</h1><p>Your devices, repair cases, and verified repair memory in one place.</p></div><div className="data-header-actions"><AccountMenu user={user} /><button className="data-icon-button" title="Notifications"><Bell size={17} /><b>{summary?.unreadNotifications || 0}</b></button><button className="data-secondary-button" onClick={loadDashboard}><RefreshCw size={15} />Refresh</button><button className="data-primary-button" onClick={() => setDeviceOpen(true)}><Plus size={15} />Add device</button></div></header>
